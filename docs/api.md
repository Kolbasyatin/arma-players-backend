...
`nickname` — имя игрока **на момент события**, а не текущее: у входов и выходов оно берётся
из сессии, у смены ника — из `payload.old`. Для `PLAYER_NICKNAME_CHANGED` это значит, что в
`nickname` лежит СТАРОЕ имя, а новое — в `payload.new`.
...
`http/api.http` — готовые запросы ко всем маршрутам с двумя окружениями (`local`, `prod`).
Токен кладётся в `http/http-client.private.env.json` (gitignored), для прода нужен SSH-туннель:
