...
func (r *CatalogRepo) SaveLobbyPage(ctx context.Context, observedAt time.Time, rooms []bohemia.Room, raw []byte, rawExpiresAt time.Time) (catalog.PageStats, error) {
	...
	for i := range rooms {
		...
		serverID, created, err := resolveOrCreateServer(ctx, tx, room, observedAt)
		if err != nil {
...
// resolveOrCreateServer ищет канонический сервер по HOST_ADDRESS; не найдя — создаёт.
// В обоих случаях обновляет «текущие» поля сервера.
func resolveOrCreateServer(ctx context.Context, tx pgx.Tx, room *bohemia.Room, observedAt time.Time) (int64, bool, error) {
	var serverID int64
	err := tx.QueryRow(ctx, `
		SELECT COALESCE(s.merged_into_server_id, s.id)
		FROM server_identity_key k
		JOIN server s ON s.id = k.server_id
		WHERE k.key_type = 'HOST_ADDRESS' AND k.key_value = $1
		ORDER BY k.last_seen_at DESC
		LIMIT 1`, room.HostAddress).Scan(&serverID)
