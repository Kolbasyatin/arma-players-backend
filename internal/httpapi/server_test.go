...
func TestServer_endpoints(t *testing.T) {
	now := time.Now().UTC()
	srv := httptest.NewServer(httpapi.NewServer(":0", fakeStatus{st: httpapi.Status{Now: now, ServersTracked: 3, PollsLastHour: map[string]int{"SUCCESS": 10}}}, nil, nil, "").Handler)
	defer srv.Close()
