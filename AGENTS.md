...
> контракт REST API — [`docs/api.md`](docs/api.md), команды телеграм-бота — [`docs/bot.md`](docs/bot.md),
> открытые задачи — [`docs/NEXT.md`](docs/NEXT.md).
...
Команды бота и шаблоны сообщений — [`docs/bot.md`](docs/bot.md).
...
### Замер 2026-09-15: база выросла до 11 ГБ

Две причины, обе исправлены (подробности и цифры — `docs/research/bohemia-api-observations.md`):

1. **Сырьё — 5,7 ГБ.** Теперь за флагом `RAW_STORE` (`off` по умолчанию, `scan`, `all`),
   удаление просроченного идёт пачками по 5000 строк, чтобы рестарт не откатывал всю работу.
2. **Распухание горячих таблиц — 4,3 ГБ.** Строк было ровно столько, сколько нужно, но каждое
   наблюдение переписывало `last_seen_at` и счётчики по каждому игроку: 45 млн обновлений
   на таблицу. Теперь служебные поля игрока трогаются не чаще раза в час, точное «последний раз
   видели» берётся из сессии, а таблицам задан `fillfactor = 85` и агрессивный автовакуум.

Правило на будущее: **в горячем пути опроса обновляется только `player_server_session`.**
Всё остальное про игрока — по изменению или по таймеру, иначе распухание возвращается.
...
# 29a. Данные Steam об избранных игроках (схема `steam`)

Необязательная тема: включается переменной `STEAM_GATEWAY_URL`. Пусто — observer работает
ровно как раньше, горутина обогащения не стартует.

**Разделение ответственности.** Ключ Steam Web API знает только `arma-reforger-hz`
(`SteamWebApi__ApiKey`), он же ходит в Valve и отдаёт сырые ответы по `GET /steam/{steamId64}`,
ничего не сохраняя. Второе место с тем же ключом создавать незачем. Хранит armaplayers.

**Отдельная схема `steam`, но та же база.** Отдельная — потому что это зеркало чужих данных:
его можно целиком выбросить и перекачать, ничего своего не потеряв. Та же база — потому что
весь смысл в джойнах к `player_platform_identity` (`game_client_type = 'PLATFORM_PC'`).

**Четыре таблицы.** `watchlist` — кого обогащаем (данные по всем игрокам не нужны).
`snapshot` — сырые ответы, источник истины. `profile` — сведённое текущее состояние из всех
четырёх источников Valve, чтобы запрос «кто с VAC-баном наиграл больше 500 часов» был одной
строкой SQL. `friend_edge` — граф дружбы: единственное, ради чего стоит раскладывать JSON,
потому что вопрос «кто из друзей тоже ходит на наши серверы» в jsonb не задаётся.

**Правила, добытые на живых данных (22.09.2026):**

- Снимок пишется, только если `body_hash` отличается от предыдущего, и хеш считается
  ПО ИСТОЧНИКАМ отдельно: `playtime_2weeks` у активного игрока меняется ежедневно, и общий
  хеш заставлял бы переписывать заодно профиль, друзей и баны. Это прямой урок `raw_payload`.
- Скрытые игры Valve отдаёт как HTTP 200 и `{"response":{}}`. Отличить «скрыто» от «нет игр»
  можно ТОЛЬКО по наличию `game_count`; судить по коду ответа нельзя.
- `GetPlayerBans` отвечает в PascalCase, остальные три метода — в snake_case.
- Разбор отделён от сохранения: сырьё ложится в базу, даже если извлечение сломалось на
  изменившейся форме. Чинится извлечение и пересчитывается по уже сохранённым снимкам.
- `SaveProfile` не затирает известные значения отсутствующими (`COALESCE` на каждом поле):
  один сбой у Valve не должен обнулять накопленное. Исключение — признаки видимости:
  они обязаны отражать последнюю попытку.
- Граф обновляется, только если список друзей ВИДЕН. На закрытом профиле пустой список
  пометил бы все связи потерянными. Пропавшие рёбра не удаляются, а получают `lost_at`.

**Частота опроса привязана к активности игрока.** Неизменные данные незачем перезапрашивать:
кто появлялся на наших серверах за `STEAM_ACTIVE_WINDOW` — обновляется раз в
`STEAM_ACTIVE_STALENESS`, остальные раз в `STEAM_IDLE_STALENESS`. Активность берётся из НАШИХ
наблюдений (`player_server_session` через `player_platform_identity`) — ещё одна причина,
по которой схема `steam` лежит в той же базе.

**Пачка размазывается по интервалу.** Шаг = `Interval / Batch`, поэтому пятьдесят игроков
за час идут по одному раз в 72 секунды, а не двумястами запросами за раз. Тот же приём,
что в `tracking`. Всплеск к Valve — повод получить 429; ровный поток проблем не создаёт.

**Список тёзок в боте знает, ради чего его открыли.** Кнопка выбора кодируется по намерению
команды: `p:` — подписаться (единственный случай, когда нажатие что-то меняет), `pc:` — карточка,
`ph:` — история, `pi:` — досье. Пока список был один на все команды, его кнопки переключали
подписку: человек спрашивал «покажи досье Zalex», выбирал из двух тёзок — и получал
«больше не слежу за Zalex». Предложение «ждать ник» тоже показывается только при подписке.

**Ссылка на профиль и SteamID показываются всегда**, в том числе когда собрать данные не удалось:
и то и другое известно из наших наблюдений, без Valve, и именно они полезнее всего при пустом
досье. Адрес строится как `/profiles/<steamid>`, если Valve не прислала своего. Полным тёзкам
в списке выбора дописываются различающие признаки (хвост Steam-аккаунта, стаж, число визитов) —
остальным не дописываются, это был бы шум.

**Досье (`GET /players/{id}/steam`) само добавляет игрока в watchlist.** Интерес человека —
лучший признак «этот игрок важен», отдельной команды «начни собирать» не нужно. При первом
обращении сбор синхронный (данных ещё нет, и показывать нечего); дальше отдаётся сохранённое,
пока оно свежее `DossierMaxAge`. Отказ шлюза не роняет ответ: устаревшие данные полезнее
пустоты, их возраст виден в `updated_at`.

**Досье по произвольному SteamID64** — `GET /steam/{steamId64}`. Steam про Arma ничего не знает,
и связь с нашим игроком необязательна: схема ключуется по `steam_id`. Если аккаунт всё-таки наш,
в ответе приходит `player_id` и игровой ник — ради этой связки обратный поиск и нужен.

**Имена полей шлюза — camelCase** (`steamId`, `fetchedAt`), а не snake_case, как в нашем API:
так сериализует ASP.NET, и клиент токена давно живёт с тем же (`accessToken`). Несовпадение
ничем себя не выдаёт при разборе — поле просто остаётся пустым, — и на проде выглядело как
`gateway answered for "", asked for ...`. Фикстуры для тестов снимать с источника, а не сочинять:
собранные вручную в неверной форме, они проверяли формат, которого не существует.

**Баны показываются двумя уровнями строгости.** VAC — это буквально «античит поймал на читах»,
утверждение однозначное. Бан от разработчика Valve выдаёт без причины: чаще всего читы, но бывает
накрутка или оскорбления, и называть человека читером на этом основании нельзя. Оба предупреждения
идут сразу под заголовком досье. К VAC обязательна оговорка, что Reforger под ним не ходит
(в категориях игры в Steam нет «Valve Anti-Cheat enabled»), иначе напрашивается вывод,
что забанен за нашу игру.

Тесты: `internal/steam/parse_test.go` (на настоящих ответах Valve в `testdata/`),
`internal/steam/client_test.go` (форма ответа шлюза),
`internal/steam/service_test.go` (досье: автодобавление, синхронный сбор, деградация),
`internal/storage/postgres/steam_repo_test.go` (дедупликация, неразрушающее обновление, граф,
разная частота для активных и неактивных).
